...
namespace boba { namespace effect { namespace generator { 
  SpikyConfig FromProtocol(const ::protocol::effect::generator::SpikyConfig& p)
  {
    ...
    if (p.has_camera_pos())
      res.cameraPos = ::boba::common::FromProtocol(p.camera_pos());

    if (p.has_camera_dir())
      res.cameraDir = ::boba::common::FromProtocol(p.camera_dir());

    if (p.has_obj_t())
      res.objT = ::boba::common::FromProtocol(p.obj_t());

    if (p.has_obj_r())
      res.objR = ::boba::common::FromProtocol(p.obj_r());
  ...
  void ToProtocol(const SpikyConfig& v, ::protocol::effect::generator::SpikyConfig* p)
  {
    ...
    ::boba::common::ToProtocol(v.cameraPos, p->mutable_camera_pos());
    ::boba::common::ToProtocol(v.cameraDir, p->mutable_camera_dir());
    ::boba::common::ToProtocol(v.objT, p->mutable_obj_t());
    ::boba::common::ToProtocol(v.objR, p->mutable_obj_r());
  }
